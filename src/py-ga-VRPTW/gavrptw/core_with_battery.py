...
def ind2route(individual, instance):
    ...
    battery_consumption_rate = instance.get('battery_consumption_rate', 1.0)
    ...
    for customer_id in individual:
        ...
        travel_distance = instance['distance_matrix'][last_customer_id][customer_id]
        updated_elapsed_time = elapsed_time + travel_distance + service_time + return_time
        battery_cost = travel_distance * battery_consumption_rate
        updated_battery = current_battery - battery_cost
        # Validate vehicle load and elapsed time
        if (updated_vehicle_load <= vehicle_capacity) and (updated_elapsed_time <= depart_due_time) and (updated_battery >= 0):
            # Add to current sub-route
            sub_route.append(customer_id)
            vehicle_load = updated_vehicle_load
            elapsed_time = updated_elapsed_time - return_time
            current_battery = updated_battery
        else:
            ...
            current_battery = battery_capacity - instance['distance_matrix'][0][customer_id] * battery_consumption_rate 
